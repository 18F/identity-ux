...
},{"field-kit":4}],2:[function(require,module,exports){
'use strict';
...
var _zipCodeFormatter = require('../app/modules/zip-code-formatter');
...
function previousAddress() {
  var accordion = window.LoginGov.accordions.filter(function (a) {
    return a.el === document.querySelector('.accordion');
  })[0];

  if (!accordion) return;

  var header = accordion.el.querySelector('.accordion-header');
  var controls = accordion.el.querySelector('[aria-controls]');
  var selects = accordion.el.querySelectorAll('select');
  var inputs = accordion.el.querySelectorAll('input');
  var originalHeading = controls.textContent;

  header.classList.remove('display-none');

  var firstExpansion = true;

  accordion.on('accordion.hide', function () {
  ...
  accordion.on('accordion.show', function () {
    controls.innerHTML = I18n.t('links.remove');

    if (firstExpansion) {
      /* eslint-disable no-new, no-shadow */
      new _fieldKit.TextField(accordion.el.querySelector('.zipcode'), new _zipCodeFormatter2.default());
...
},{"../app/modules/zip-code-formatter":1,"classlist.js":3,"field-kit":4}],3:[function(require,module,exports){
/*
